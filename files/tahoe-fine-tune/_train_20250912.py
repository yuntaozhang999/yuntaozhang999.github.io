...
def run_tx_train(cfg: DictConfig):
    ...
    from lightning.pytorch.callbacks import LearningRateMonitor  # new
    ...
    class _OptParamsOnceLogger(pl.Callback):
        """Log optimizer static hyperparameters (WD, betas, eps, etc.) as one-time metrics at the start of training."""
        def on_train_start(self, trainer, pl_module):
            if not trainer.optimizers:
                return
            opt = trainer.optimizers[0]
            g0 = opt.param_groups[0] if opt.param_groups else {}
            # Log these once as "hyperparameter metrics" (written to W&B and metrics.csv)
    ...
    class _LrPerGroupLogger(pl.Callback):
        """Log LR for each param_group at every step; allows monitoring real-time LR even without a scheduler."""
    ...
    # Logging learning rate and optimizer parameters (controlled by training.log_lr in config)
    ...
    if checkpoint_path is None and manual_init is not None:
        ...
        # Only load tensors to avoid deserializing unnecessary objects (requires torch>=2.1; False works for older versions)
        ...
        # Keep only weights where keys exist and shapes match exactly; skip the rest
        ...
        # **Crucial**: Do not let Lightning restore the optimizer/scheduler
        checkpoint_path = None

        # Optional: Print real LR before training starts for verification
